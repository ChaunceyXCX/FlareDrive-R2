...
如果你希望直接从本地环境部署到 Cloudflare Pages，可以使用我们提供的部署命令：

1. 确保已安装 Node.js 和 pnpm，在项目根目录安装依赖：
   ```bash
   pnpm install
   ```
...
2. 运行部署命令：
   ...
   pnpm run deploy
   ```
   *注意：首次运行会要求你登录 Cloudflare 账号，请根据终端提示在浏览器中完成授权。部署完成后，记得依然需要完成上述的环境变量配置和存储桶绑定（步骤 3 和 4）。*
